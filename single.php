...
/*--------------------------------------------------------------------------------------------------
	ACTION BOX AREA
--------------------------------------------------------------------------------------------------*/
	mefe_get_template_from_area ('action_box_area',$post->ID,$meta_fields);
		...
		<div class="container">
			...
			<div id="mainbody">
				...
				<?php while(have_posts()) : the_post(); 
					...
					// GET POST OPTIONS 
					$post_meta_fields = get_post_meta($post->ID, 'mefe_meta_elements', true);
					...
					if ( isset( $meta_fields['page_builder_layout'] ) && $meta_fields['page_builder_layout'] == 'style1' ) {
						$row_style = ' row-fluid mefe_def_margin mefe_has_sidebar';
					}
					...
					if ( empty ( $post_meta_fields['page_layout'] ) || empty ( $post_meta_fields['sidebar_select'] ) ) {
						if ( $data['default_sidebar_position'] == 'left_sidebar' ) {
							$content_css = 'span9 mefe_float_right mefe_content mefe_has_sidebar';
							$sidebar_css = 'sidebar-left';
							$has_sidebar = true;
							$mainbody_css = 'mefe_has_sidebar';
						}
						elseif ( $data['default_sidebar_position'] == 'right_sidebar' ) {
							$content_css = 'span9 mefe_content mefe_has_sidebar';
					...
					if ( has_post_thumbnail() && ! post_password_required() ) {
						...
						if ( isset( $data['sb_use_full_image'] ) && $data['sb_use_full_image'] == 'yes' )  {
							...
							$image = '<div class="mefe_full_image"><a data-rel="prettyPhoto" href="'.$f_image.'" class="hoverBorder">'.get_the_post_thumbnail($post->ID ,'full-width-image').'</a></div>';
				
						}
						else {
							$image = '<div class="mefe_post_image"><a data-rel="prettyPhoto" href="'.$f_image.'" class="hoverBorder pull-left">'.get_the_post_thumbnail().'</a></div>';
						}
					}
...
--------------------------------------------------------------------------------------------------*/
						if ( isset ( $meta_fields['content_main_area'] ) && is_array ( $meta_fields['content_main_area'] ) && ( isset( $meta_fields['page_builder_layout'] ) && $meta_fields['page_builder_layout'] == 'style1' ) ) {
							mefe_get_template_from_area ('content_main_area',$post->ID,$meta_fields,true);
						}
...
--------------------------------------------------------------------------------------------------*/
	if ( isset ( $meta_fields['content_main_area'] ) && is_array ( $meta_fields['content_main_area'] ) && ( !isset( $meta_fields['page_builder_layout'] ) || $meta_fields['page_builder_layout'] == 'default' ) ) {
		echo '<div class="container">';
			mefe_get_template_from_area ('content_main_area',$post->ID,$meta_fields);
		echo '</div>';
	}
...
--------------------------------------------------------------------------------------------------*/
	...
	echo '<div class="gray-area '.$cls.'">';
		echo '<div class="container">';
			...
			mefe_get_template_from_area ('content_grey_area',$post->ID,$meta_fields);
...
--------------------------------------------------------------------------------------------------*/
	...
	if ( isset ( $meta_fields['content_bottom_area'] ) && is_array ( $meta_fields['content_bottom_area'] ) ) {
		echo '<div class="container">';
			mefe_get_template_from_area ('content_bottom_area',$post->ID,$meta_fields);
		echo '</div>';
	}
?>
