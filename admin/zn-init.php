...
// Load the interface
	...
	include(locate_template(array('admin/framework/class-zn-html.php'), false));
	include(locate_template(array('admin/framework/functions-zn-admin.php'), false));
	

/*--------------------------------------------------------------------------------------------------
	Load all METABOX RELATED FILES
--------------------------------------------------------------------------------------------------*/

	function load_metaboxes() 
	{	

		include(locate_template(array('admin/framework/function-zn-metabox.php'), false));
	    return new mefe_metabox();
	}

    add_action('load-post.php', 'load_metaboxes');
	add_action('load-post-new.php', 'load_metaboxes');


	require_once dirname( __FILE__ ) . '/framework/class-tgm-plugin-activation.php';
	add_action( 'tgmpa_register', 'mefe_register_required_plugins' );

	function mefe_register_required_plugins() {
		...
		$plugins = array(
			array(
				...
				'source'   				=> ADMIN_MASTER_ADMIN_DIR . 'plugins/revslider.zip', // The plugin source
			...
			array(
				...
				'source'   				=> ADMIN_MASTER_ADMIN_DIR . 'plugins/cutesliderwp.zip', // The plugin source
				'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
		...
		$config = array(
			...
			'parent_menu_slug' 	=> THEMENAME, 				// Default parent menu slug
