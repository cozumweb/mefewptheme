...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_set_options' ) ) {
	function mefe_set_options() {
		...
		$import_code = mefe_replace_image_links_with_local( $import_code );
		
		update_option('mefe_mefe_options',$import_code);
...
/*--------------------------------------------------------------------------------------------------
	ZN SET PROPER MEDIA
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_replace_image_links_with_local' ) ) {
	function mefe_replace_image_links_with_local( $zarray ) {
		//$new_array = array ();
				...
				if ( !is_array( $val ) ) {
					// FUNCTIA DE SCHIMBAT URL SI UPLOAD POZA IN FOLDERUL WP-CONTENT
						...
						if ( isImage ( $val ) ) {
							...
							$image_path_on_upload = explode( 'http://hogash-demo.com/mefe_wp/wp-content/uploads/',$val);
					...
					$new_array[$key] = mefe_replace_image_links_with_local( $val );
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_update_featured' ) ) {
	function mefe_update_featured( $id )
	{
		global $sample_image_id;
