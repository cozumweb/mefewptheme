...
// Contains the meta types/pages/options
 ...
 var $meta_fields;
	...
	function mefe_metabox (){
	
		global $mefe_meta_types,$mefe_meta_elements;

		include_once(locate_template(array('admin/options/zn-meta-boxes.php'), false));
	
		$this->meta_types = $mefe_meta_types;
		$this->meta_elements = $mefe_meta_elements;
		$this->html = new mefe_html();
		
		add_action('add_meta_boxes', array(&$this, 'mefe_init_options'));
		add_action('save_post', array(&$this, 'mefe_save_options'));
		
	}


	function mefe_init_options ()
	{
		...
		foreach($this->meta_types as $key=>$type) 
		{
			foreach ($type['page'] as $page ) 
			{
				...
				add_meta_box( 
					$type['id'],
					$type['title'],
					array($this,'mefe_render_meta_box'),
					$page,
					$type['context'],
					$type['priority'],	
					array('what_box'=>$type)
	...
	*
	*****************************************************************/
	function mefe_render_meta_box ( $post , $metabox ) 
	{
		// Get the current metabox
		$mefe_metabox = $metabox['args']['what_box'];
		...
		if( !isset ( $element['link_to'] ) ) { $element['link_to'] = '';}
			...
			if( $element['link_to'] == $mefe_metabox['id']) 
			{
					...
					{	
						...
						if ( $element['type'] == 'mefe_dynamic_list' ) {
							...
							$output .= '<div class="mefe_meta_box mefe_meta_box'.$element['type'].' meta_box_'.$mefe_metabox['context'].'">';
							

								$output .= $this->html->mefe_render_element($element);
						...
						elseif(isset($meta_fields[$element['id']]))
						{
							...
							$output .= '<div class="mefe_meta_box mefe_meta_box_'.$element['type'].' meta_box_'.$mefe_metabox['context'].'">';
	...
	*
	*****************************************************************/
	function mefe_save_options( $post_id ) {
		...
		if(isset($_POST['post_ID'])) 
		{
			...
			if ( isset ( $_POST['security'] ) ) {
				...
				if (! wp_verify_nonce($nonce, 'mefe_ajax_nonce') ) return; 
			}
			else {
				return;
			}
...
/*
		...
		*/
			...
			// Save them already :)
			update_post_meta($post_id, 'mefe_meta_elements', $values_to_save);
