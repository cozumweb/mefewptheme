...
***************************************************/
	...
	function mefe_show_options($options,$menu = null) 
	{
		...
		foreach ( $options as $option ) 
		{
			...
			/*
			if ( $option['type'] == 'mefe_dynamic_list' ) {
			...
			$output .= $this->mefe_render_element($option);
		}
	
		$output .= $this->mefe_page_end();
	...
	function mefe_show_menu($menu_items){
		...
		foreach($menu_items as $menu)
		{
			...
			if(isset($menu['submenus'])) {
				...
				$output .= '<li class="parent"><a href="#" rel="">'.$menu['name'].'</a>'.$this->mefe_show_menu($menu['submenus']).'</li>';
			
			}
			else
			{
				$output .= '<li><a class="normal '.$menu['class'].'" href="#mefe_page_'.$menu['id'].'" rel="">'.$menu['name'].'</a></li>';
			}
	...
	function mefe_page_start() {
		
		global $mefe_has_error;
		
		$output  = '';
		
		if ( is_array ( $mefe_has_error ) && !empty ( $mefe_has_error[0] ) ) {
			$output .= '<div id="mefe_error_container">';
				//$output .= '<h3>Error :</h3>';
				foreach ($mefe_has_error as $error ){
					$output .= $error.'<br/>';
				}
			$output .= '</div>';
		}
		
		$output  .= '<div id="mefe_options_container">';

		
		// Start Form
		$output .= '<form id="mefe_form" method="post" action="'. esc_attr( $_SERVER['REQUEST_URI'] ) .'" enctype="multipart/form-data" >';
		...
		$output .= $this->mefe_show_menu($this->menu);
		$output .= '</div>';
		...
		//$output .= '<button type="button" class="mefe_save button-primary">Save All Changes</button>';
		$output .= '<button type="button" class="mefe_save mefe_save_button"></button>';
		...
		$output .= '<div class="h_info"><a target="_blank" href="http://mefesoft.com/">Get Support</a></div>';

		$output .= '<div class="h_info"><a target="_blank" href="http://mefesoft.com/">Online Documentation</a></div>';
	...
	function mefe_page_end() {
		
		$output   = '';

		$output  .= '</div>'; // End container
		
		$output .= '<div id="mefe_footer"> <div id="js-warning">Warning- This options panel will not work properly without javascript!</div>';
		...
		//$output .= '<button type="button" class="mefe_save button-primary">Save All Changes</button>';
		$output .= '<button type="button" class="mefe_save mefe_save_button"></button>';
		$output  .= '<input type="hidden" id="security" name="security" value="'.wp_create_nonce('mefe_ajax_nonce').'" />';
		$output  .= '<input type="hidden" name="mefe_reset" value="reset" />';
		$output .= '</div>';
...
* Is called by the mefe_show_options() and group()
***************************************************/

	function mefe_render_element($option){
		...
		{
			$output = '<div class="mefe_page" id="mefe_page_'.$option['id'].'">';
		}
		...
		if($option['type'] == 'mefe_accordion_start')
		{
			
			$class = $option['class'];
			
			$output .= '<div class="mefe_single_toggle mefe_slide_header '.$class.'"><strong>'.$option['name'].'</strong><div class="mefe_slide_buttons"><a class="mefe_slide_edit_button" href="#">a</a></div></div><div class="mefe_slide_body '.$class.'" >'."\n";
			
			
		}
		
		if($option['type'] == 'mefe_accordion_end')
		{
		...
		if(method_exists( $this, $option['type'] ) ) 
		{
			...
			elseif($option['type'] == 'group')
			{
				...
				$not_sortable = 'mefe_sortable';
				if ( !$option['group_sortable'] ) {
					$not_sortable = '';
				}
				
				$output .= '<div class="mefe_accordion  '.$option['class'].'">';
				$output .= '<h4 class="heading">'.$option['name'].'</h4>';
				$output .= '<ul id="'.$option['id'].'"  class="'.$not_sortable.' mefe_normal_group">';
				
				$output .= $this->$option['type']( $option ) ;
				
				$output .= '</ul>';
				$output .= '<div class="clear"></div>';
				$output .= '<a class="mefe_slide_add_button zn-blue" href="#">Add a new '.$option['add_text'].'</a>';
				$output .= '<span class="mefe_add_type">'.$option['id'].'</span>';
				$output .= '<div class="clear"></div>';
				$output .= '</div>';
			}
			...
			else
			{
				...
				$output .= $this->mefe_section_start( $option );
				
				$output .= $this->$option['type']( $option );
			
					
				$output .= $this->mefe_section_end( $option );
	...
	function group ($element) {
		...
		*/
		...
		for ($i = 0; $i < $number_of_elements; $i++) {
			...
			$output .= '<li class="mefe_slide '.$size.'">';
			$output .= '<div class="mefe_slide_header '.$handle.'">';
			...
			// Slide buttons
			$output .= '<div class="mefe_slide_buttons">';
			...
			if (  isset ( $element['is_dynamic'] ) && $element['is_dynamic'] ) 
			{
				...
				$output .= '<div class="mefe_size_container">';
				
				$output .= '<a href="#" class="mefe_slide_decrease_button '.$first_inactive.'"> - </a>';
				
				$output .= '<a href="#" class="mefe_slide_increase_button '.$last_inactive.'"> + </a>';
				$output .= '</div>';
				// Slide buttons : edit , delete and size
				$output .= '<div href="#" class="mefe_slide_size" data-sizes="'.$element['sizes'].'">'.$mefe_size.'</div>';
			}
			

			$output .= '<a href="#" class="mefe_slide_edit_button">a</a>';
			$output .= '<a href="#" class="mefe_slide_delete_button">d</a>';
			$output .= '</div>';
			
			if (  isset ( $element['is_dynamic'] ) && $element['is_dynamic'] ) 
			{
				$output .= '<a href="#" class="mefe_slide_close_button" title="Done">Done</a>';
			}
			
			$output .= '</div>';
			
			$output .= '<div class="mefe_slide_body">';
			...
			foreach($element['subelements'] as $key => $value) {
				...
				$output .=  $this->mefe_render_element($value);
...
*				Dynamic list Option
***************************************************/
	function mefe_dynamic_list($element) {
	...
	/*	
		...
		echo '</pre>';
	*/	
		$output .= '<div class="mefe_dynamic_accordion">';
		...
		$output .= '<div class="mefe_page_area">';
			$output .=  '<div class="mefe_pb_area_name"><h4>'.$area['area_name'].'</h4>';
		
			if( !isset ( $area['limit'] ) ) { $area['limit'] = '';}
		
			$output .= '<a href="#" data-pbarea="'.$area_id.'" data-limit="'.$area['limit'].'" class="mefe_slide_add_button zn-blue mefe_inactive zn-gray">Add Element</a>';
			$output .= '<div class="select_wrapper">';
				$output .= '<select class="select mefe_select_dynamic zn-input" >';
			...
			$output .= '<span class="mefe_hider mefe_add_type"></span>';

			$output .= '<div class="clear"></div>';
			
			$output .= '</div>';
		
			$output .= '<ul id="'.$area_id.'"  class="mefe_dynamic_list_container">';
				...
				foreach($element['std'] as $key=>$value) 
				{
					...
					if ( isset ( $element['std'][$key]['pb_area'] ) && $element['std'][$key]['pb_area'] == $area_id ) {
						...
						$option = mefe_get_element_from_id($new_element);
...
***************************************************/
	...
	function mefe_backup_restore ( $value ){
	
		$output = '';


		$output .= '<div class="mefe_backup"><input name="mefe_backup_options" type="submit" value="Backup theme options" class="mefe_backup_options zn-blue "></div>';
		$output .= '<div class="mefe_backup_sep"></div>';
			...
			if ( count( $theme_backups ) > 0 ){
				...
				$output .= '<span class="mefe_grey_text">Backup list :</span></br>';
				$output .= '<ul class="mefe_backup_list">';
				foreach ($theme_backups as $key => $value) {
					$option_name = $value->option_name;
					$output .= '<li><a class="mefe_question mefe_restore_option" data-backup="'.$option_name.'" original-title="Restore this backup ?" href="#">'. $option_name.'</a><span data-backup="'.$option_name.'" class="mefe_question mefe_delete_backup mefe_delete" original-title="Delete this backup ?">x</span><li>';
				}
				$output .= '</ul>';
				$output .= '<span class="mefe_grey_text no_backups_text" style="display:none;">No backups created so far. Press the "<b>Backup theme options</b>" button from above to perform a backup.</span>';
			}
			else {
				$output .= '<ul class="mefe_backup_list">';
				$output .= '</ul>';
				$output .= '<span class="mefe_grey_text no_backups_text">No backups created so far. Press the "<b>Backup theme options</b>" button from above to perform a backup.</span>';
			}
...
***************************************************/
	...
	function one_click_install ( $value ){
			...
			if ( get_option(THEMENAME.'_dummy') == 1 ) {
				...
				$disabled = 'disabled';
				$css = 'mefe_inactive zn-gray';
			}
			
			$output .= '<input name="mefe_install_dummy_data" type="submit" value="Install dummy data " class="mefe_install_dummy zn-blue '.$css.'" '.$disabled.'/> ';
			$output .= '<input type="hidden" id="security" name="security" value="'.wp_create_nonce('mefe_ajax_nonce').'" />';
		...
		$output .= '<div class="mefe_response"></div>';
...
***************************************************/
	...
	function date_picker ( $value ){
		...
		$output = '<label for="'. $value['id'].'[date]">Date:</label><input class="zn-input mefe_date_picker" name="'.$value['id'].'[date]" id="'. $value['id'].'[date]" value="'. $date_val .'" type="text" /><label for="'. $value['id'].'[time]">Time :</label><input id="'. $value['id'].'[time]" name="'. $value['id'].'[time]" value="'. $time_val .'" type="text" class="mefe_time_picker" />';
...
***************************************************/
	...
	// echo '</pre>';
		...
		$output .= '<input class="mefe_upload_image_button" type="button" value="Upload Image" />';
		...
		if ( isset($value['alt'] ) ) {
			$output .= '<input class="mefe_img_alt" id="'.$value['id'].'_alt" type="hidden" name="'.$value['id'].'[alt]" value="'.$saved_alt.'" />';
			$output .= '<input class="mefe_img_title" id="'.$value['id'].'_title" type="hidden" name="'.$value['id'].'[title]" value="'.$saved_title.'" />';
		}
...
***************************************************/
	...
	function mefe_radio ( $value )
	{
		...
		$output = '';
		$output .= '<div id="'.$value['id'].'" class="mefe_radio">';
		$i = 0;
...
***************************************************/
	...
	function mefe_hide_select ( $value )
	{
		...
		$output = '';
	$output .= '<div id="'.$value['id'].'" class="mefe_hide">';
...
***************************************************/
	function image_size ( $value ) 
	{
		...
		$output .= '<div class="mefe_image_size">';
		$output .= '<div>';
...
***************************************************/
	...
	function mefe_g_fonts( $value ) {
		$output = '';

		


		$output .= '<div class="mefe_accordion">';
		$output .= '<h4 class="heading">'.$value['name'].'</h4>';
		$output .= '<ul id="'.$value['id'].'"  class=" mefe_normal_group">';
	    ...
	    if ( !empty($value['fonts']) ) {
			...
			$output .= '<li class="mefe_slide ">';
			$output .= '<div class="mefe_slide_header">';
...
***************************************************/
	function select ( $value ){
		...
		*/
		if( $value['mod'] == 'multi' ) {
			$output .= '<select class="select mefe_multi zn-input" title="Select an option" multiple="multiple" name="'.$value['id'].'[]" id="'. $value['id'] .'">';
			foreach ($value['options'] as $select_ID => $option) {
...
/*
		...
		echo '</pre>';
*/	
		$output = '<div class="mefe_font_icon_container">';
		$output .= '<input class="mefe_font_icon_input" value="'. $value['std'] .'" name="'.$value['id'].'" type="hidden"/>';
		$output .= '<input type="button" value="Select Icon" class="mefe_select_icon_font">';
		...
		{
			$output .= '<span class="mefe_icon_holder '.$icon_name.'">'.$icon_name.'</span>';
		}*/
...
*/
/*
	function mefe_section_start( $option ){
	...
	function mefe_section_end( $option ){
			...
			if( $option['type'] != 'hidden' )
			{
				...
				if( $option['description'] )
				{
					$output .= '<div class="mefe_title_and_description"><div class="mefe_title"><h4 class="heading">'. $option['name'] .'</h4></div><div class="zn-description">'.$option['description'].'</div></div>';
				}
	...
	*/
	...
	function mefe_section_start( $option ){
			...
			if( $option['type'] != 'hidden' )
			{
				...
				if( $option['description'] )
				{
					$output .= '<div class="mefe_title_and_description"><div class="mefe_title"><h4 class="heading">'. $option['name'] .'<span class="mefe_question" '.$title.'>?</span></h4></div></div>';
				}
...
***************************************************/
	...
	*/	
		...
		$output = '<div class="mefe_mp">';
		
		$output .= '<div class="mefe_nop">';
			$output .= '<span class="option_title">Columns :</span>';
			$output .= '<ul class="mefe_number_list">';
		...
		$output .= '<div class="mefe_positions">';
		
		$output .= '<div class="mefe_positions_display">';
			...
			for ( $i=1; $i<$number_of_columns+1; $i++) {
				...
				$output .= '<div class="mefe_position zn-grid-'.$saved_variation.' '.$css.'"><span>'.$alphabet[$i-1].'</span></div>';
			}
			$output .= '</div>';
			$output .= '<div class="clear"></div>';
			$output .= '<div class="mefe_position_options">';
			
				// All position variations
				$output .= '<div class="mefe_position_var_options">';
				...
				// All position variations
				$output .= '<div class="mefe_all_options hidden">';
			...
			$output .= '<div class="clear"></div>';
			// Positions input
			$output .= '<input class="mefe_widgets_positions hidden" data-columns="'.key($saved_widgets_array).'" name="'.$value['id'].'" id="'.$value['id'].'" type="text" value="'. htmlspecialchars ($saved_widgets_display ) .'" />';
