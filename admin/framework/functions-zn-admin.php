...
function mefe_get_default_values() 
{
	...
	foreach ( $mefe_options as $option)
	{
...
**************************************************/
function mefe_admin_init()
{	
	
	if (!get_option(OPTIONS)){
		
		$defaults = mefe_get_default_values();
...
if (is_admin() && isset($_GET['activated'] ) && $pagenow == "themes.php" ) {
	...
	add_action('admin_head','mefe_admin_init');
}
...
add_action('init', 'mefe_sc_button');


function mefe_sc_button() {
...
function register_button( $buttons ) {
	array_push( $buttons, "|", "mefe_button" );
	return $buttons;
}

function add_plugin( $plugin_array ) {
   $plugin_array['mefe_button'] = get_template_directory_uri() . '/admin/js/mefe_sc_button.js';
   return $plugin_array;
}

function mefe_sc_dialog() {
...
$i = '0';
	...
	<div id="mefe_sidebar">
		<div id="zn-nav">
			<ul class="mefe_activate_nav">
				<?php
					foreach ( $categories as $name => $shortcodes ) {
						$cls = '';
						if ( $i == '0') { $cls = 'active'; }
						echo '<li><a rel="" href="#mefe_page_'.$name.'" class="normal '.$cls.'">'.$name.'</a></li>';
					
						
							$page .= '<div id="mefe_page_'.$name.'" class="mefe_page">';
								$page .= '<h4 class="heading">'.$name.'</h4>';
								foreach ( $shortcodes as $shortcode_name => $shortcode_value ) {
									$page .= '<div class="mefe_sc_container"><div class="mefe_sc_title">'.$shortcode_name.'</div><div class="mefe_shortcode_text">'.$shortcode_value.'</div></div>';
								}
							$page .= '</div>';
...
/*-----------------------------------------------------------------------------------*/
/* ZN Framework Admin Interface - mefe_framework_add_admin */
/*-----------------------------------------------------------------------------------*/

function mefe_framework_add_admin() {
	
    $mefe_page = add_menu_page(THEMENAME, THEMENAME . ' Options', 'edit_theme_options', THEMENAME,'mefe_framework_options_page',ADMIN_IMAGES_DIR.'/favicon.png');
    $mefe_woo_options = add_submenu_page( THEMENAME, 'Woocommerce Options', 'Woocommerce Options', 'edit_theme_options', 'mefe_woo_page', 'mefe_woo_options_page' );


	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$mefe_page", 'of_load_only');
	add_action("admin_print_scripts-$mefe_woo_options", 'of_load_only');
	add_action("admin_print_styles-$mefe_page",'of_style_only');
	add_action("admin_print_styles-$mefe_woo_options",'of_style_only');
	...
	{	
		of_load_only();
		of_style_only();
		add_action('admin_footer', 'mefe_sc_dialog');
	}
	
	
} 

add_action('admin_menu', 'mefe_framework_add_admin');

/*-----------------------------------------------------------------------------------*/
/* Build the Options Page  */
/*-----------------------------------------------------------------------------------*/

function mefe_framework_options_page(){

	$html = new mefe_html();

	global $mefe_options, $mefe_admin_menu;
	include(locate_template(array('admin/options/theme-options.php'), false));
	echo $html->mefe_show_options($mefe_options ,$mefe_admin_menu ); 
...
function mefe_woo_options_page(){
	$html = new mefe_html();
			...
			var doc = {
				ready: function(){
					...
					jQuery('.zn-remove-image').live('click',function(){
						jQuery(this).parents('.controls').children('.mefe_upload_image_button').prev().val('');
			...
			slider = {
				...
				init: function(){
					// bind the button's click the browse_clicked handler
						...
						$('.mefe_upload_image_button').live("click", 	slider.browse_clicked);
				...
				browse_clicked: function(event){
					...
					slider.field_alt = jQuery(this).parent().find('.mefe_img_alt');
					slider.field_title = jQuery(this).parent().find('.mefe_img_title');
						...
						if (slider.field !=false) {
							...
							if ( attachment.filename.match(/.jpg$|.jpeg$|.ico$|.png$|.gif$/) ) {
								...
								slider.field_title.val(attachment.title);
									...
									image = '<a href="#" class="zn-remove-image">remove</a><img class="mefe_mu_image" src="'+imgurl+'" alt="" />';
...
/*-----------------------------------------------------------------------------------*/
function mefe_get_element_from_id( $id ) {

	// INCLUDE the options files
	global $mefe_meta_elements,$extra_options,$mefe_options;
	include(locate_template(array('admin/options/zn-meta-boxes.php'), false));
	include(locate_template(array('admin/options/theme-options.php'), false));
		
	$options = array_merge($mefe_options,$mefe_meta_elements,$extra_options);	
...
function generate_options_css($newdata) {
	...
	if(is_multisite()) {
		$mefe_uploads_dir = trailingslashit($uploads['basedir']);
	} else {
		$mefe_uploads_dir = $css_dir;
	}
	...
	file_put_contents($mefe_uploads_dir . 'options.css', $css);
...
/*-----------------------------------------------------------------------------------*/
	if ( !empty( $data['mefe_theme_username'] ) && !empty( $data['mefe_theme_api'] ) ) {

		$username = $data['mefe_theme_username'];
		$apikey = $data['mefe_theme_api'];
		$author = 'MefeSOFT';
...
$css_file = $mefe_uploads_dir.'options.css';

if ( !is_writable($css_file) ) {
	$mefe_has_error[] = '<br/>Please make sure that the bellow files exists and have the proper file permissions ( 644 or 777 ) and belong to the same user group as the rest of the wordpress files. Until you set the proper permissions to those files , you won\'n be able to change any color/bacgrounds <br/>';
	$mefe_has_error[] = $css_file.' <b>IS NOT WRITEABLE OR DOESN\'T EXISTS</b>';

	$mefe_has_error[] = '<br/><h3>HOW TO RESOLVE THIS :</h3>';
	$mefe_has_error[] = '+ If this is the first time you use the theme or you\'ve just updated the theme , save the options and refresh the page<br/>';
	$mefe_has_error[] = '+ If the above step didn\'t removed this message, using a FTP client, please check that the above mentioned files are present in the mentioned directories. If the files are not present, please manually create them and then save the options again and refresh the page.';
...
--------------------------------------------------------------------------------------------------*/
function mefe_permalink_settings_init() {
	...
	// Add our settings
	add_settings_field(
		'mefe_portfolio_item_slug_input',      	// id
		'Portfolio item slug', 	// setting title
		'mefe_portfolio_item_slug',  // display callback
	...
	// Add our settings
	add_settings_field(
		'mefe_portfolio_taxonomy_slug_input',      	// id
		'Portfolio taxonomy slug', 	// setting title
		'mefe_portfolio_taxonomy_slug',  // display callback
		'permalink',                 				// settings page
		'zn-portfolio-permalink'                  				// settings section
	);
	...
	// Add our settings
	add_settings_field(
		'mefe_doc_item_slug_input',      	// id
		'Documentation taxonomy slug', 	// setting title
		'mefe_doc_item_slug',  // display callback
		'permalink',                 				// settings page
	...
	// Add our settings
	add_settings_field(
		'mefe_doc_taxonomy_slug_input',      	// id
		'Documentation taxonomy slug', 	// setting title
		'mefe_doc_taxonomy_slug',  // display callback
		'permalink',                 				// settings page
		'zn-doc-permalink'                  				// settings section
	);
...
add_action( 'admin_init', 'mefe_permalink_settings_init' );

/*--------------------------------------------------------------------------------------------------
	Permalinks actual options
--------------------------------------------------------------------------------------------------*/
function mefe_portfolio_item_slug() {
	$permalinks = get_option( 'mefe_permalinks' );
	?>
	<input name="mefe_portfolio_item_slug_input" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['port_item'] ) ) echo esc_attr( $permalinks['port_item'] ); ?>" placeholder="portfolio" />
	<?php
}

function mefe_portfolio_taxonomy_slug() {
	$permalinks = get_option( 'mefe_permalinks' );
	?>
	<input name="mefe_portfolio_taxonomy_slug_input" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['port_tax'] ) ) echo esc_attr( $permalinks['port_tax'] ); ?>" placeholder="project_category" />
	<?php
}

function mefe_doc_item_slug() {
	$permalinks = get_option( 'mefe_permalinks' );
	?>
	<input name="mefe_doc_item_slug_input" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['doc_item'] ) ) echo esc_attr( $permalinks['doc_item'] ); ?>" placeholder="documentation" />
	<?php
}

function mefe_doc_taxonomy_slug() {
	$permalinks = get_option( 'mefe_permalinks' );
	?>
	<input name="mefe_doc_taxonomy_slug_input" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['doc_tax'] ) ) echo esc_attr( $permalinks['doc_tax'] ); ?>" placeholder="documentation_category" />
	<?php
}
...
--------------------------------------------------------------------------------------------------*/
function mefe_permalink_settings_save() {
	if ( ! is_admin() )
		return;

	// We need to save the options ourselves; settings api does not trigger save for the permalinks page
	if ( isset( $_POST['permalink_structure'] ) || isset( $_POST['mefe_portfolio_item_slug_input'] ) ) {
		// Portfolio items slug
		$mefe_portfolio_item_slug_input = sanitize_text_field( $_POST['mefe_portfolio_item_slug_input'] );
		$mefe_portfolio_taxonomy_slug_input = sanitize_text_field( $_POST['mefe_portfolio_taxonomy_slug_input'] );

		// Documentatin items slug
		$mefe_doc_item_slug_input = sanitize_text_field( $_POST['mefe_doc_item_slug_input'] );
		$mefe_doc_taxonomy_slug_input = sanitize_text_field( $_POST['mefe_doc_taxonomy_slug_input'] );
		...
		$permalinks['port_item'] 	= untrailingslashit( $mefe_portfolio_item_slug_input );
		$permalinks['port_tax'] 	= untrailingslashit( $mefe_portfolio_taxonomy_slug_input );
		$permalinks['doc_item'] 	= untrailingslashit( $mefe_doc_item_slug_input );
		$permalinks['doc_tax'] 	= untrailingslashit( $mefe_doc_taxonomy_slug_input );

		update_option( 'mefe_permalinks', $permalinks );
	}
}

add_action( 'admin_init', 'mefe_permalink_settings_save' );

/*--------------------------------------------------------------------------------------------------
	AJAX CALLBACK
--------------------------------------------------------------------------------------------------*/
add_action('wp_ajax_mefe_ajax_post_action', 'mefe_ajax_callback');

function mefe_ajax_callback() {

	$nonce=$_POST['security'];
	
	if (! wp_verify_nonce($nonce, 'mefe_ajax_nonce') ) die('-1'); 
	...
	elseif ($save_type == 'mefe_restore_options') {
	...
	elseif ($save_type == 'mefe_delete_backup') {
	...
	elseif ($save_type == 'install_dummy') {
		...
		locate_template(array('admin/dummy_content/mefe_importer.php'), true, true);
		installDummy();

	} 
	elseif ($save_type == 'mefe_backup_options') {
	...
	elseif ($save_type == 'add_element') {
		...
		// All subelements options must be placed in the same array that is passed to mefe_get_element_from_id() function in functions-zn-admin.php !!
		...
		$option = mefe_get_element_from_id($data['element_type']);
		...
		echo $html->mefe_render_element($option); 
