					...
					if ( has_post_thumbnail(  ) && ! post_password_required() ) {
						...
						if ( !empty ( $f_image ) ) {
							if ( isset( $data['sb_archive_use_full_image'] ) && $data['sb_archive_use_full_image'] == 'yes' )  {
								...
								$image = '<div class="mefe_post_image"><a href="'.get_permalink().'" class="hoverBorder pull-left">'.get_the_post_thumbnail().'</a></div>';
							}
						}

					}
					elseif ( $data['mefe_use_first_image'] == 'yes' && ! post_password_required()) {
						...
						if ( !empty ( $f_image ) ) {
							...
							if ( isset( $data['sb_archive_use_full_image'] ) && $data['sb_archive_use_full_image'] == 'yes' )  {
								...
								$size = mefe_get_size( 'sixteen' ,$has_sidebar , 30 );

								
								$image = vt_resize( '', $f_image  , $size['width'] ,'' , true );
								$image = '<div class="mefe_full_image"><a data-rel="prettyPhoto" href="'.$feature_image.'" class="hoverBorder"><img class="shadow" src="'.$image['url'].'" alt=""/></a></div>';
