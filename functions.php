...
--------------------------------------------------------------------------------------------------*/
	...
	define( 'THEMENAME', 'mefe' ); 
	define( 'OPTIONS', 'mefe_mefe_options' ); 
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_portfolio_taxonomy_pagination' ) ) {
	function mefe_portfolio_taxonomy_pagination( $query ) {
...
add_action( 'pre_get_posts', 'mefe_portfolio_taxonomy_pagination' );

/*--------------------------------------------------------------------------------------------------
	Show the page builder elements based on area
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_get_template_from_area' ) ) {
	function mefe_get_template_from_area ($area,$post_id,$meta_fields,$sidebar = false)
	{
...
--------------------------------------------------------------------------------------------------*/
add_filter('the_content', 'mefe_wrap_images');
if ( ! function_exists( 'mefe_wrap_images' ) ) {
	function mefe_wrap_images ($content)
	{	global $post;
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_get_size' ) ) {
	function mefe_get_size( $size , $sidebar = null , $extra = 0 ) {
		global $data;
		$span_sizes = array (
			"four"=>"span3", 
			"one-third"=>"span4", 
...
if ( ! function_exists( 'mefe_adjust_content_width' ) ) {
	function mefe_adjust_content_width() {
		global $content_width,$data;

		// 1170 LAYOUT
		if ( $data['mefe_width'] == '960' ) {
	...
	add_action( 'template_redirect', 'mefe_adjust_content_width' );
...
if ( ! function_exists( 'mefe_custom_gallery' ) ) {
	function mefe_custom_gallery($attr) {
		global $post;
		...
		$c = 1;
			...
			foreach ( $attachments as $id => $attachment ) {
				...
				if ( $c == 1 || $c % ($columns+1) == 0 ) {
					$output .= '<div class="row-fluid mefe_image_gallery">';
					$c = 1;
				}
...
add_shortcode('gallery', 'mefe_custom_gallery');

	//add_theme_support('post-formats', array( 'aside', 'chat', 'gallery', 'image', 'link', 'quote', 'status', 'video', 'audio'));
/*--------------------------------------------------------------------------------------------------
	Logo SEO function
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_logo' ) ) {
	function mefe_logo(){ 
...
/*--------------------------------------------------------------------------------------------------
	Favicon function
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_favicon' ) ) {
	function mefe_favicon(){ 
	...
	add_action('wp_head','mefe_favicon'); 
	
/*--------------------------------------------------------------------------------------------------
	Load the css for the theme
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_add_styles' ) ) {
	function mefe_add_styles()
	{
		global $data;
		...
		if (!is_admin())
		{
			...
			/*	Load Google Fonts if they are needed */
				...
				wp_enqueue_style( 'mefe_all_g_fonts', '//fonts.googleapis.com/css?family='.$gfont.''.$subset);



				
			if ( $data['mefe_main_style'] == 'dark' ) {
	...
	add_action('wp_enqueue_scripts','mefe_add_styles');
			...
			<?php
				...
				if (!empty( $data['face_og'] ) ) {
					?>
						...
						<meta property="og:title" content="<?php mefe_opengraph_default_title();?>"/>
						<meta property="og:type" content="<?php mefe_opengraph_default_type();?>"/>
...
add_action('wp_head','mefe_head');

/*--------------------------------------------------------------------------------------------------
	ZN PAGE LOADING
--------------------------------------------------------------------------------------------------*/	
if ( ! function_exists( 'mefe_page_loading' ) ) {
	function mefe_page_loading() {
		global $data;
...
if ( ! function_exists( 'mefe_flag_content_and_sidebar' ) ) {
	function mefe_flag_content_and_sidebar() {
		global $content_and_sidebar;
	...
	function smart_load() {
		...
		if ( $post ) {
			...
			if ( is_archive() && get_post_type() == 'product' ) {
				...
				$meta_fields = get_post_meta($shop_page_id, 'mefe_meta_elements', true);
				$meta_fields = maybe_unserialize($meta_fields);
			}
			else {
				$meta_fields = get_post_meta($post->ID, 'mefe_meta_elements', true);
				$meta_fields = maybe_unserialize( $meta_fields );
			}
			...
			foreach ($metas as $options ) 
			{
				...
				if  ( $options['dynamic_element_type'] == '_content_sidebar' ) {
					mefe_flag_content_and_sidebar();
				}
				
				// Load Paralax effect
				if  ( $options['dynamic_element_type'] == '_rev_slider' ) {
					if(  $options['revslider_paralax'] ){
						wp_enqueue_script('mefe_parallax', MASTER_THEME_DIR . '/addons/paralax/parallax.js',array('jquery'),'1.3',true);
					}
				}
				...
				if  ( $options['dynamic_element_type'] == '_nivoslider' ) {
					...
					wp_enqueue_style('mefe_shadows', MASTER_THEME_DIR . '/css/shadows.css', 'style'); 
				...
				if  ( $options['dynamic_element_type'] == '_static6' ) {
					wp_enqueue_script('mefe_product_loupe', MASTER_THEME_DIR . '/addons/jquery_loupe/jquery.loupe.min.js',array('jquery'),'1.3',true);
				}
				...
				if  ( $options['dynamic_element_type'] == '_static7' ) {
					wp_enqueue_script('mefe_event_countdown', MASTER_THEME_DIR . '/js/jquery.countdown.js',array('jquery'),'1.3',true);
				}
				...
				if ( is_active_widget( false, false, 'mefe_mailchimp', true )  ) {
					$mefe_mailchimp = array ( 'mefe_mailchimp' =>
						...
						jQuery('.nl-submit').click(function() {
							...
							result_placeholder = jQuery(this).parent().next('span.mefe_mailchimp_result');
							...
							jQuery.ajax({
								...
								type: 'POST',
								data: {
									mefe_mc_email: jQuery(this).prevAll('.nl-email').val(),
									mefe_mailchimp_list: jQuery(this).prev('.nl-lid').val(),
									mefe_ajax: '' // Change here with something different
								},
					...
					mefe_update_array( $mefe_mailchimp );
				...
				$mefe_isotope = array ( 'mefe_isotope' =>
						"
						(function($){ 
						...
						mefe_update_array( $mefe_isotope );
				...
				if  ( is_post_type_archive('portfolio') && $data['portfolio_style'] == 'portfolio_carousel' ) {
					...
					$mefe_pcarousel = array ( 'mefe_pcarousel' =>
						"
							(function($) {
								$(window).load(function(){
									// ** Portfolio Carousel
						...
						mefe_update_array( $mefe_pcarousel );
				}
		}
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_add_script' ) ) {
		...
		if (!is_admin()){
			...
			if ($data['menu_follow'] == 'yes') {
				...
				$mefe_chaser = array ( 'mefe_chaser' =>
						...
						mefe_update_array( $mefe_chaser );

			}

			if ( $data['page_preloader'] == 'yes' ) {

				$mefe_preloader = array ( 'mefe_preloader' =>
						"
						(function($){ 
						...
						mefe_update_array( $mefe_preloader );

			}
			
			// RESPONSIVE MENU
			if (  $data['res_menu_style'] == 'select' && $data['mefe_responsive'] == 'yes'  ) {

				$mefe_smooth_menu = array ( 'mefe_smooth_menu' =>
						"
						(function($){ 
							$(window).load(function(){
						...
						mefe_update_array( $mefe_smooth_menu );

			}
			elseif ( $data['res_menu_style'] == 'smooth' && $data['mefe_responsive'] == 'yes' ) {
			...
			wp_register_style('mefe_shadows', MASTER_THEME_DIR . '/css/shadows.css', 'style'); 
			...
			if ( is_active_widget( false, false, 'tp_widget_recent_tweets', true )  ) {
					...
					$twitter_fader = array ( 'mefe_twitter_fader' =>
							"							
					...
					mefe_update_array( $twitter_fader );
			...
			if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
				...
				if ( $data['woo_show_cart'] ) {
					...
					$mefe_woo_cart = array ( 'mefe_woo_cart' =>
						"
							(function($) {
								$(window).load(function(){
									$('body').bind('added_to_cart',function (evt,ret) { 
										...
										var mycart = $('#mycartbtn'), // trebuie adaugat id-ul
											...
											$(placeholderdiv).fadeIn('slow', 'easeInOutExpo',function(){
												...
												var mefe_pos_top = $(this).offset().top,
													mefe_pos_left = $(this).offset().left;

													$(this).css({margin:0,left:mefe_pos_left,top:mefe_pos_top,position:\"absolute\"}).delay(800).animate({\"top\": mycartTop, left:mycartLeft, opacity:1}, 2000, 'easeInOutExpo',function(){
													$(this).remove();
						...
						mefe_update_array( $mefe_woo_cart );
				}
			}
			global $pagenow;
			// Event Countdown
			if  ( $data['cs_enable'] == 'yes' && !is_user_logged_in() && !is_admin() && $pagenow != 'wp-login.php' ) {
				...
				$mefe_event_countdown_coming_soon = array ( 'mefe_event_countdown_coming_soon' =>
						"
						(function($) {
							var counter = {
								init: function (d)
						...
						mefe_update_array( $mefe_event_countdown_coming_soon );
			...
			wp_localize_script( 'zn-script', 'mefe_do_login', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
			
		}
	}
}
	add_action('wp_enqueue_scripts','mefe_add_script');
	
/*--------------------------------------------------------------------------------------------------
	Update inline scripts
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_update_array' ) ) {
	function mefe_update_array($script) {
		
		$mefe_array = wp_cache_get('mefe_array');
		$mefe_array[key ($script)] = $script[key ($script)];
		wp_cache_set('mefe_array',$mefe_array);
...
if ( ! function_exists( 'mefe_load_inline_js' ) ) {
	function mefe_load_inline_js($script) {
		
		$mefe_array = wp_cache_get('mefe_array');

		$script = '<script type="text/javascript">';
			foreach ( $mefe_array as $tt ){
				$script .= $tt;
			}
		$script .= '</script>';
...
add_action('wp_footer', 'mefe_load_inline_js', 100);
...
--------------------------------------------------------------------------------------------------*/
	...
	add_action( 'init', 'mefe_register_menu' );
if ( ! function_exists( 'mefe_register_menu' ) ) {
	function mefe_register_menu() {
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_show_nav' ) ) {
	function mefe_show_nav($location) {
...
function hh_update_nav_menu_objects( $menu_id = null, $menu_data = null ) {
	delete_transient( 'mefe_menu_transient' );
}
...
--------------------------------------------------------------------------------------------------*/
add_filter( 'nav_menu_css_class', 'mefe_active_item_classes', 10, 2 );
if ( ! function_exists( 'mefe_active_item_classes' ) ) {
	function mefe_active_item_classes($classes = array(), $menu_item = false){
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_comment' ) ) {
	function mefe_comment($comment, $args, $depth) {
...
--------------------------------------------------------------------------------------------------*/
if ( !function_exists('mefe_hidden_pannel') ) {
	function mefe_hidden_pannel(){
...
--------------------------------------------------------------------------------------------------*/
	...
	add_filter('get_search_form', 'mefe_search_form');
if ( ! function_exists( 'mefe_search_form' ) ) {	 
	function mefe_search_form($form) {
	$form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
		<div class="mefe_search_container">
...
if ( ! function_exists( 'mefe_limit_content' ) ) {	
	function mefe_limit_content($string, $word_limit)
	{
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_pagination' ) ) {	
	function mefe_pagination($pages = '', $range = 2)
	{  
		$showitems = ($range * 2)+1;  
		...
		//__( 'Published in', THEMENAME )
			...
			for ($i=1; $i <= $pages; $i++)
			{
				...
				{
					//echo ($paged == $i)? "<span class='current mefe_default_color_active'>".$i."</span>":"<a href='".get_pagenum_link($i)."' class='inactive mefe_default_color' >".$i."</a>";
...
--------------------------------------------------------------------------------------------------*/
	...
	add_action('init','mefe_portfolio_post_type');
if ( ! function_exists( 'mefe_portfolio_post_type' ) ) {		
	function mefe_portfolio_post_type() {
	
		$permalinks = get_option( 'mefe_permalinks' );
	...
	// Flush Rewrite rules
if ( ! function_exists( 'mefe_rewrite_flush' ) ) {	
	function mefe_rewrite_flush() {
		flush_rewrite_rules();
	}
}
	
	add_action( 'after_switch_theme', 'mefe_rewrite_flush' );	

/*--------------------------------------------------------------------------------------------------
	Portfolio Post Taxonomy
--------------------------------------------------------------------------------------------------*/

	add_action( 'init', 'mefe_portfolio_category', 0 );
if ( ! function_exists( 'mefe_portfolio_category' ) ) {	
	function mefe_portfolio_category() 
	{
...
/*--------------------------------------------------------------------------------------------------
	Breadcrumbs
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_breadcrumbs' ) ) {					
	function mefe_breadcrumbs() {
		...
		elseif ( is_home() ) {
			...
			if ( is_category() ) {
				...
				if ($thisCat->parent != 0) $cats = get_category_parents($thisCat->parent, TRUE, '|mefe_preg|');

				$cats = get_category_parents($thisCat, TRUE, '|mefe_preg|');
				
				$cats = explode ( '|mefe_preg|',$cats );
			...
			elseif ( is_single() && !is_attachment() ) {
				if ( get_post_type() == 'portfolio' ) {
					...
					// Show category name
					$cats = get_the_term_list($post->ID, 'project_category', ' ', '|mefe_preg|', '|mefe_preg|');
					$cats = explode ( '|mefe_preg|',$cats );
				...
				elseif ( get_post_type() != 'post' ) {
					...
					$cats = get_category_parents($cat, TRUE, '|mefe_preg|');
...
/*--------------------------------------------------------------------------------------------------
	Login Form
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_login_form' ) ) {	
	function mefe_login_form () {
	...
	<div class="login_register_stuff hide"><!-- Login/Register Modal forms - hidded by default to be opened through modal -->
		<div id="login_panel">
			<div class="inner-container login-panel">
			 <?php // wp_login_form(  ); ?> 
				...
				<form id="login_form" name="login_form" method="post" class="mefe_form_login" action="<?php echo site_url('wp-login.php', 'login_post') ?>">
					...
					<label class="mefe_remember"><input type="checkbox" name="rememberme" id="rememberme" value="forever"><?php _e(" Remember Me",THEMENAME);?></label>
					<input type="submit" id="login" name="submit_button" class="mefe_sub_button" value="<?php _e("LOG IN",THEMENAME);?>">
					<input type="hidden" value="login" class="" name="mefe_form_action">
					<input type="hidden" value="mefe_do_login" class="" name="action">
					<input type="hidden" value="<?php echo $current_url; ?>" class="mefe_login_redirect" name="submit">
		...
		<?php if( get_option('users_can_register') ) { ?>
			<div id="register_panel">
				<div class="inner-container register-panel">
					...
					<form id="register_form" name="login_form" method="post" class="mefe_form_login" action="<?php echo site_url('wp-login.php?action=register', 'login_post') ?>">
						<p>
						...
						<p>
							<input type="submit" id="signup" name="submit" class="mefe_sub_button" value="<?php _e("CREATE MY ACCOUNT",THEMENAME);?>">
						</p>
						<input type="hidden" value="register" class="" name="mefe_form_action">
						<input type="hidden" value="mefe_do_login" class="" name="action">
						<input type="hidden" value="<?php echo $current_url; ?>" class="mefe_login_redirect" name="submit">
		...
		<div id="forgot_panel">
			<div class="inner-container forgot-panel">
				...
				<form id="forgot_form" name="login_form" method="post" class="mefe_form_lost_pass" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>">
					<p>
						<input type="text" id="forgot-email" name="user_login" class="inputbox" placeholder="<?php _e("Username or E-mail",THEMENAME);?>">
					</p>
					<p>
						<input type="submit" id="recover" name="submit" class="mefe_sub_button" value="<?php _e("SEND MY DETAILS!",THEMENAME);?>">
					</p>
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_stop_redirecting' ) ) {	
    function mefe_stop_redirecting($redirect_to, $request,$user) {
	...
	add_filter("login_redirect", "mefe_stop_redirecting", 10, 3);
...
--------------------------------------------------------------------------------------------------*/
/* DEFAULT TITLE*/
if ( ! function_exists( 'mefe_opengraph_default_title' ) ) {	
	function mefe_opengraph_default_title() {
...
if ( ! function_exists( 'mefe_opengraph_default_type' ) ) {	
	function mefe_opengraph_default_type() {
...
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_coming_soon_page' ) ) {
	function mefe_coming_soon_page() {
...
add_action('init', 'mefe_coming_soon_page',26);
...
if ( ! function_exists( 'mefe_has_ajax' ) ) {
	function mefe_has_ajax() {
		global $has_ajax;
			...
			if ( $post ) {
				...
				if  ( $options['dynamic_element_type'] == '_c_form' ) {
					if ( isset ($_POST['cform_submit']) ) {
						...
						foreach ( $options['mefe_cf_fields'] as $field ) {
							
							$field_name = preg_replace('~[\W\s]~', '_', $field['mefe_cf_name']);
							// Compoese the mail
							if ( isset ( $_POST[$field_name] ) ) {
							
								$body .= $field['mefe_cf_name'] .' : '.$_POST[$field_name] ."<br />" ;
								if ( $field['mefe_cf_f_email'] ) {
									$email = $_POST[$field_name];
								}
								if ( $field['mefe_cf_f_name'] ) {
									$name = $_POST[$field_name];
								}
							}

							if ( $field['mefe_cf_type'] == 'captcha' ){
								$captcha = true;
							}
						...
						if ( $captcha && !empty($data['rec_priv_key']) ){
							...
							// What happens when the CAPTCHA was entered incorrectly
								...
								if ( wp_mail ($options['mefe_cf_email_address'],$options['mefe_cf_button_subject'],$body,$headers) ){
									echo 'sent';
								}
								else {
									echo 'mail_not_sent';
	...
	add_action('get_header', 'mefe_has_ajax');
...
/*--------------------------------------------------------------------------------------------------
	New TOP NAVIGATION
--------------------------------------------------------------------------------------------------*/
	add_action( 'init', 'mefe_register_menu2' );
if ( ! function_exists( 'mefe_register_menu2' ) ) {
	function mefe_register_menu2() {
		if ( function_exists('wp_nav_menu') ) {
			add_theme_support( 'nav-menus' );
			register_nav_menus( array(
...
--------------------------------------------------------------------------------------------------*/	
	...
	add_filter('body_class','mefe_body_class_names');
if ( ! function_exists( 'mefe_body_class_names' ) ) {
	function mefe_body_class_names($classes) {
		global $data;
		if ( (!empty ( $data['mefe_boxed_layout'] ) && $data['mefe_boxed_layout'] == 'yes' ) || ( is_front_page() && !empty( $data['mefe_home_boxed_layout'] ) && $data['mefe_home_boxed_layout'] =='yes' )) {

			$classes[] = 'boxed';
			
		}
		
		if ( is_front_page() && !empty( $data['mefe_home_boxed_layout'] ) && $data['mefe_home_boxed_layout'] =='no' ){
			$classes = array_diff($classes, array("boxed"));
		}
		
		if ($data['mefe_width'] == '1170' &&  $data['mefe_responsive'] == 'yes') {
		...
		if ( !empty($data['mefe_slider_header']) && $data['mefe_slider_header'] == 'yes')  {
...
--------------------------------------------------------------------------------------------------*/	
add_action( 'mefe_after_body', 'mefe_add_page_loading',10 );
if ( !function_exists('mefe_add_page_loading') ) {
	function mefe_add_page_loading(){
		echo mefe_page_loading();
	}
}

/* SUPPORT PANEL */
add_action( 'mefe_after_body', 'mefe_add_hidden_panel',10 );
if ( !function_exists('mefe_add_hidden_panel') ) {
	function mefe_add_hidden_panel(){
		echo mefe_hidden_pannel();
	}
}

/* LOGIN FORM */
add_action( 'mefe_after_body', 'mefe_add_login_form',10 );
if ( !function_exists('mefe_add_login_form') ) {
	function mefe_add_login_form(){
		echo mefe_login_form();
	}
}

/* OPEN GRAPH */
add_action( 'mefe_after_body', 'mefe_add_open_graph',10 );
if ( !function_exists('mefe_add_open_graph') ) {
	function mefe_add_open_graph(){
		echo mefe_f_o_g();
	}
}
...
--------------------------------------------------------------------------------------------------*/	
add_action( 'mefe_head_right_area', 'mefe_add_navigation',40 );
if ( !function_exists('mefe_add_navigation') ) {
	function mefe_add_navigation(){
		echo  mefe_show_nav('header_navigation');
	}
}
...
--------------------------------------------------------------------------------------------------*/	
add_action( 'mefe_head_right_area', 'mefe_hidden_pannel_link',30 );
if ( !function_exists('mefe_hidden_pannel_link') ) {
	function mefe_hidden_pannel_link(){
...
--------------------------------------------------------------------------------------------------*/
add_action( 'mefe_head_right_area', 'mefe_login_text',20 );

if ( ! function_exists( 'mefe_login_text' ) ) {	
	function mefe_login_text() {
		global $data;
...
--------------------------------------------------------------------------------------------------*/
add_action( 'mefe_head_right_area', 'mefe_header_social_icons',10 );

if ( ! function_exists( 'mefe_header_social_icons' ) ) {	
	function mefe_header_social_icons() {
		global $data;
...
--------------------------------------------------------------------------------------------------*/	
add_action( 'mefe_head_right_area', 'mefe_woocomerce_cart',2 );

if ( ! function_exists( 'mefe_woocomerce_cart' ) ) {	
	function mefe_woocomerce_cart () {
		include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
...
--------------------------------------------------------------------------------------------------*/
add_action( 'mefe_head_right_area', 'mefe_wpml_language_switcher',3 );

if ( ! function_exists( 'mefe_wpml_language_switcher' ) ) {	
	function mefe_wpml_language_switcher () {
...
--------------------------------------------------------------------------------------------------*/
	...
	add_action('init','mefe_documentation_post_type');
if ( ! function_exists( 'mefe_documentation_post_type' ) ) {		
	function mefe_documentation_post_type() {
...
--------------------------------------------------------------------------------------------------*/
	...
	add_action( 'init', 'mefe_documentation_category', 0 );
if ( ! function_exists( 'mefe_documentation_category' ) ) {	
	function mefe_documentation_category() 
	{
...
/*--------------------------------------------------------------------------------------------------
	LOGIN SYSTEM
--------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'mefe_do_login' ) ) {		
	function mefe_do_login()
	{

		if ( $_POST['mefe_form_action'] == 'login' ) {
			$user = wp_signon();
			if ( is_wp_error($user) ) {
		...
		elseif( $_POST['mefe_form_action'] == 'register' ){

			$mefe_error = false;
			$mefe_error_message = array();

			if ( !empty( $_POST['user_login'] ) ) {
				if ( username_exists( $_POST['user_login'] ) ){	
					$mefe_error = true;
					$mefe_error_message[] = __('The username already exists',THEMENAME);
				}
				else {
				...
				$mefe_error = true;
				$mefe_error_message[] = __('Please enter an username',THEMENAME);
			}
			...
			if ( !empty( $_POST['user_password'] ) ) {
				...
				$mefe_error = true;
				$mefe_error_message[] = __('Please enter a password',THEMENAME);
			}

			if ( ( empty( $_POST['user_password'] ) && empty( $_POST['user_password2'] ) ) || $_POST['user_password'] != $_POST['user_password2'] ) {
				$mefe_error = true;
				$mefe_error_message[] = __('Passwords do not match',THEMENAME);
			}
			...
			if ( !empty( $_POST['user_email'] ) ) {
				...
				if( !email_exists( $_POST['user_email'] )) {
					if (!filter_var($_POST['user_email'], FILTER_VALIDATE_EMAIL)) {
						$mefe_error = true;
						$mefe_error_message[] = __('Please enter a valid EMAIL address',THEMENAME);
					}
					else{
					...
					$mefe_error = true;
					$mefe_error_message[] = __('This email address has already been used',THEMENAME);
				}
				
			}
			else {
				$mefe_error = true;
				$mefe_error_message[] = __('Please enter an email address',THEMENAME);
			}
			...
			if ( $mefe_error ){
				...
				foreach ( $mefe_error_message as $error) {
					echo $error.'<br />';
				}
				echo '</div>';
		...
		elseif( $_POST['mefe_form_action'] == 'reset_pass' ){
...
add_action("wp_ajax_nopriv_mefe_do_login", "mefe_do_login");
add_action("wp_ajax_mefe_do_login", "mefe_do_login");
