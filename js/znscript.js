...
;(function($){
	...
	$(document).ready(function(e) {
		...
		// LOGIN FORM
		jQuery(document).on('submit','.mefe_form_login',function(event){
			event.preventDefault();
		
			var form = $(this),
				warning = false,
				button = $('.mefe_sub_button',this),
			...
			if( warning ) {
				button.removeClass('mefe_blocked');
				return false;
			}
			
			if( button.hasClass('mefe_blocked')) {
				return;
			}
			
			button.addClass('mefe_blocked');
	
			jQuery.post(mefe_do_login.ajaxurl, values, function(resp) {
				...
				if ( $('#login_error', data).length ) {
					...
					if ( $('.mefe_login_redirect', form).length > 0 ) {
						jQuery.prettyPhoto.close();
						redirect = $('.mefe_login_redirect', form);
						href = redirect.val();
		...
		// LOST PASSWORD
		jQuery(document).on('submit','.mefe_form_lost_pass',function(event){
			event.preventDefault();
    ...
    $('.mefe_menu_trigger').unbind('click')
...
*/
	...
	--------------------------------------------------------------------------------------------------*/
		if($('#slideshow').hasClass('slider_fixedd')) {
			...
			$('.mefe_fixed_slider_fill').css({'height':parseInt($('#slideshow').css('height'))-css+'px', 'background-color': $('body').css('background-color')});
