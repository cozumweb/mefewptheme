...
if ($data['mefe_width'] == '1170' &&  $data['mefe_responsive'] == 'no') {

	echo 'body{min-width:1200px;}';

}
elseif ($data['mefe_width'] == '960' &&  $data['mefe_responsive'] == 'no'){
...
#page_header.mefe_def_header_style , #slideshow.mefe_def_header_style , #page_header.uh_mefe_def_header_style , #slideshow.uh_mefe_def_header_style{
<?php if ( isset ( $data['def_header_color'] ) && !empty ( $data['def_header_color'] ) ) { echo 'background-color:'.$data['def_header_color'].';'; } ?>
}

#page_header.mefe_def_header_style #sparkles, #slideshow.mefe_def_header_style #sparkles , #page_header.uh_mefe_def_header_style #sparkles, #slideshow.uh_mefe_def_header_style #sparkles {
<?php if ($data['def_header_animate'] == '1' ) { echo 'display:block;'; } ?>
}

#page_header.mefe_def_header_style .bgback , #slideshow.mefe_def_header_style .bgback , #page_header.uh_mefe_def_header_style .bgback , #slideshow.uh_mefe_def_header_style .bgback{
...
<?php
	...
	echo '#page_header.mefe_def_header_style , #slideshow.mefe_def_header_style {';
				// GRADIENT OVER COLOR
	...
	if ( isset ( $data['def_glare'] ) && !empty ( $data['def_glare'] ) ) {
		...
		echo '#page_header.mefe_def_header_style .bgback:after , #slideshow.mefe_def_header_style .bgback:after {';
	...
	if ( isset ( $data['def_bottom_style'] ) && $data['def_bottom_style'] == 'shadow' ) {
		...
		echo '#page_header.mefe_def_header_style .mefe_header_bottom_style , #slideshow.mefe_def_header_style .mefe_header_bottom_style {';
		...
		echo '#page_header.mefe_def_header_style .mefe_header_bottom_style:after , #slideshow.mefe_def_header_style .mefe_header_bottom_style:after {';
	...
	if ( isset ( $data['def_bottom_style'] ) && $data['def_bottom_style'] == 'shadow_ud' ) {
		...
		echo '#page_header.mefe_def_header_style .mefe_header_bottom_style:before , #slideshow.mefe_def_header_style .mefe_header_bottom_style:before {';
	...
	// MASK 1
	if ( isset ( $data['def_bottom_style'] ) && $data['def_bottom_style'] == 'mask1' && $data['mefe_main_style'] == 'light' ) {
	...
	elseif ( isset ( $data['def_bottom_style'] ) && $data['def_bottom_style'] == 'mask1' && $data['mefe_main_style'] == 'dark' )  {
		echo '#page_header.mefe_def_header_style .mefe_header_bottom_style , #slideshow.mefe_def_header_style .mefe_header_bottom_style {';
	...
	// MASK 2
	if ( isset ( $data['def_bottom_style'] ) && $data['def_bottom_style'] == 'mask2' && $data['mefe_main_style'] == 'light' ) {
	...
	elseif ( isset ( $data['def_bottom_style'] ) && $data['def_bottom_style'] == 'mask2' && $data['mefe_main_style'] == 'dark' ) {
		echo '#page_header.mefe_def_header_style .mefe_header_bottom_style , #slideshow.mefe_def_header_style .mefe_header_bottom_style {';
			echo 'position:absolute; bottom:0; left:0; width:100%;  z-index:99; ';
		...
		foreach ( $data['header_generator'] as $header ) {
			...
			if ( isset ( $header['uh_style_name'] ) && !empty ( $header['uh_style_name'] ) ) {
				...
				if ( isset ( $header['uh_bottom_style'] ) && $header['uh_bottom_style'] == 'shadow' ) {
					...
					echo '#page_header.uh_'.$header_name.' .mefe_header_bottom_style , #slideshow.uh_'.$header_name.' .mefe_header_bottom_style {';
						echo 'position:absolute; bottom:0; left:0; width:100%; height:20px; background:url('.get_template_directory_uri().'/images/shadow-up.png) no-repeat center bottom; z-index: 2;';
					echo '}';
					
					echo '#page_header.uh_'.$header_name.' .mefe_header_bottom_style:after , #slideshow.uh_'.$header_name.' .mefe_header_bottom_style:after {';
						echo 'content:\'\'; position:absolute; bottom:-18px; left:50%; border:6px solid transparent; border-top-color:#fff; margin-left:-6px;';
					echo '}';
				...
				if ( isset ( $header['uh_bottom_style'] ) && $header['uh_bottom_style'] == 'shadow_ud' ) {
					...
					echo '#page_header.uh_'.$header_name.' .mefe_header_bottom_style:before , #slideshow.uh_'.$header_name.' .mefe_header_bottom_style:before {';
						echo 'content:\'\'; position:absolute; bottom:-26px; left:0; width:100%; height:20px; background:url('.get_template_directory_uri().'/images/shadow-down.png) no-repeat center top; opacity:.6; filter:alpha(opacity=60);';
					echo '}';
				...
				// MASK 1
				if ( isset ( $header['uh_bottom_style'] ) && $header['uh_bottom_style'] == 'mask1' && $data['mefe_main_style'] == 'light' ) {
				...
				elseif ( isset ( $header['uh_bottom_style'] ) && $header['uh_bottom_style'] == 'mask1' && $data['mefe_main_style'] == 'dark' )  {
					echo '#page_header.uh_'.$header_name.' .mefe_header_bottom_style , #slideshow.uh_'.$header_name.' .mefe_header_bottom_style {';
						echo 'position:absolute; bottom:0; left:0; width:100%; height:27px; z-index:99; background:url('.get_template_directory_uri().'/images/bottom_mask_dark.png) no-repeat center top;';
					echo '}';
				}
				...
				// MASK 2
				if ( isset ( $header['uh_bottom_style'] ) && $header['uh_bottom_style'] == 'mask2' && $data['mefe_main_style'] == 'light' ) {
				...
				elseif ( isset ( $header['uh_bottom_style'] ) && $header['uh_bottom_style'] == 'mask2' && $data['mefe_main_style'] == 'dark' ) {
					echo '#page_header.uh_'.$header_name.' .mefe_header_bottom_style , #slideshow.uh_'.$header_name.' .mefe_header_bottom_style {';
						echo 'position:absolute; bottom:0; left:0; width:100%;  z-index:99; ';
						echo 'height:33px; background:url('.get_template_directory_uri().'/images/bottom_mask2_dark.png) no-repeat center top;';
					echo '}';
...
/* GENERAL COLOR */
	...
	{color:<?php echo $data['mefe_main_color']; ?>;}
	...
	{border-top: 3px solid <?php echo $data['mefe_main_color'];?>;}
...
#bbpress-forums div.bbp-search-form input[type=submit], #bbpress-forums .bbp-submit-wrapper button, #bbpress-forums #bbp-your-profile fieldset.submit button {background-color:<?php echo $data['mefe_main_color'];?>;}

.breadcrumbs li:after {
	border-left-color: <?php echo $data['mefe_main_color'];?>;
}

span.zonsale:before ,#limited_offers li:before{border-color:<?php echo $data['mefe_main_color'];?>  transparent transparent;}

	.how_to_shop .number, .newsletter-signup input[type=submit], .vertical_tabs .tabbable .nav>li.active>a>span, .vertical_tabs .tabbable .nav>li>a:hover>span, #map_controls, .hg-portfolio-sortable #portfolio-nav li.current a, .ptcarousel .controls > a:hover, .itemLinks span a:hover, .product-list-item .details .actions a, .shop-features .shop-feature:hover, .btn-flat, .redbtn, #sidebar ul.menu li a:hover, .imgboxes_style1 .hoverBorder h6, .feature_box.style3 .box:hover, .services_box .box:hover .icon, .latest_posts.default-style .hoverBorder h6, .process_steps .step.intro, .recentwork_carousel.style2 li a .details .plus, .gobox.ok, .hover-box:hover, .recentwork_carousel li .details > .bg, .circlehover:before,.iosSlider .item .caption.style1 .more:before, .iosSlider .item .caption.style1 .more:after ,.iosSlider .item .caption.style2 .more {background-color:<?php echo $data['mefe_main_color'];?>;}
	#action_box:before ,#action_box:after, header#header.style1{border-top-color:<?php echo $data['mefe_main_color'];?>;}
	...
	.theHoverBorder:hover {box-shadow:0 0 0 5px <?php echo $data['mefe_main_color'];?> inset;}

	.offline-page .containerbox {border-bottom:5px solid <?php echo $data['mefe_main_color'];?>; }

	.offline-page .containerbox:after {border-top: 20px solid <?php echo $data['mefe_main_color'];?>;}

	header#header.style2 #logo a {border-top: 3px solid <?php echo $data['mefe_main_color'];?>;}

	body .iosSlider .item .caption.style2 .title_big, body .iosSlider .item .caption.style2 .title_small {border-left: 5px solid <?php echo $data['mefe_main_color'];?>; }
	body .iosSlider .item .caption.style2.fromright .title_big, body .iosSlider .item .caption.style2.fromright .title_small {border-right: 5px solid <?php echo $data['mefe_main_color'];?> ; }

/* Buddypress styles */
#buddypress form#whats-new-form p.activity-greeting:after {border-top-color: <?php echo $data['mefe_main_color'];?>;}

#buddypress input[type=submit],
#buddypress input[type=button],
#buddypress input[type=reset] ,
#buddypress .activity-list li.load-more a {background: <?php echo $data['mefe_main_color'];?>;}

#buddypress div.item-list-tabs ul li.selected a,
#buddypress div.item-list-tabs ul li.current a {border-top: 2px solid <?php echo $data['mefe_main_color'];?>;}

#buddypress form#whats-new-form p.activity-greeting,
.widget.buddypress ul.item-list li:hover {background-color: <?php echo $data['mefe_main_color'];?>;}
...
#buddypress .acomment-options a {color:<?php echo $data['mefe_main_color'];?>;}
...
body #page_wrapper , body.boxed #page_wrapper {
	<?php if ( !empty ( $data['mefe_body_def_color'] ) ) { echo 'background-color:'.$data['mefe_body_def_color'].';'; } ?>
...
.gray-area {
	<?php if ( !empty ( $data['mefe_gr_area_def_color'] ) ) { echo 'background-color:'.$data['mefe_gr_area_def_color'].';'; } ?>
...
<?php 
if ( !empty ( $data['mefe_boxed_layout'] ) && $data['mefe_boxed_layout'] == 'yes' ) {
	?>
	body {
...
<?php
	...
	if ( !empty($data['mefe_top_nav_color']) ) {
		echo '.topnav > li > a { color:'.$data['mefe_top_nav_color'].' ;}';
	}
	if ( !empty($data['mefe_top_nav_h_color']) ) {
		echo '.topnav > li > a:hover { color:'.$data['mefe_top_nav_h_color'].' ;}';
	}
...
/* WOOCOMMERCE CATEGORY IMAGE */
	...
	if ( !empty ( $data['mefe_custom_css'] ) ) {
		echo  stripslashes ( $data['mefe_custom_css'] );
	}
?>