...
<?php
	...
	if ( is_home() || ( is_archive() && get_post_type() == 'post' ) ) {
		...
		global $wp_query;
		$meta_fields = get_post_meta($page_id, 'mefe_meta_elements', true);
		$meta_fields = maybe_unserialize($meta_fields);
	}
	...
	$header_css = '';
	if ( !empty($meta_fields['mefe_disable_subheader']) && $meta_fields['mefe_disable_subheader'] == 'yes' ) {
		$header_css = 'no_subheader';
	}
	elseif ( !empty($meta_fields['mefe_disable_subheader']) && $meta_fields['mefe_disable_subheader'] == 'no' ) {
		$header_css = '';	
	}
	elseif( ( !empty( $data['mefe_disable_subheader'] ) && $data['mefe_disable_subheader'] == 'yes' ) ) {
		$header_css = 'no_subheader';
	}
?>

    <div id="page_wrapper">
        <header id="header" class="<?php echo $data['mefe_header_layout'] .' '. $header_css; ?>">
            <div class="container">
                <!-- logo -->
                <?php echo mefe_logo(); ?>
								
				<!-- HEADER ACTION -->
				<?php do_action( 'mefe_head_right_area' ); ?>
				...
				<nav id="main_menu" class="<?php if ( !empty ( $data['res_menu_style'] ) &&  $data['res_menu_style'] == 'smooth' ) { echo 'smooth_menu'; } ?>">
					<?php if ( !empty ( $data['res_menu_style'] ) &&  $data['res_menu_style'] == 'smooth' ) { echo '<div class="mefe_menu_trigger"><a href="">'.__('Menu',THEMENAME).'</a></div>'; } ?>
					<?php mefe_show_nav('main_navigation'); ?>
				</nav>
				<!-- end main_menu -->
			</div>
		</header>
...
--------------------------------------------------------------------------------------------------*/
	...
	elseif ( !empty($meta_fields['mefe_disable_subheader']) && $meta_fields['mefe_disable_subheader'] == 'no' && isset ( $meta_fields['header_area']) && is_array($meta_fields['header_area'] ) ) {
	    mefe_get_template_from_area('header_area', $post -> ID, $meta_fields);
		return;
	}
	elseif ( !empty($meta_fields['mefe_disable_subheader']) && $meta_fields['mefe_disable_subheader'] == 'no' ) {
	...
	elseif(is_404()) {
	   ...
	   ?>
		<div id="page_header" class="<?php echo $style; ?> bottom-shadow">
			...
			<div class="mefe_header_bottom_style"></div>
	   <?php
	   return;
	}
	elseif ( is_post_type_archive( 'documentation' ) || is_tax( 'documentation_category' ) || 'documentation' == get_post_type() ){
		...
		$style = '';
		if ( !empty($data['mefe_doc_header_style']) ) {
			$style = 'uh_'.$data['mefe_doc_header_style'];
		}


		?>
			<div id="page_header" class="<?php echo $style; ?> mefe_documentation_page" >
				<div class="bgback"></div>
	...
	<div id="page_header" class="<?php echo $style; ?>">
				<div class="bgback"></div>
					...
					<div class="container">
						<div class="row">
							<div class="span6">
								<?php mefe_breadcrumbs();	?>
							</div>
							<div class="span6">
								<div class="header-titles">
									<h2>Showcase</h2>
									<h4>How our clients used mefe to build websites</h4>
								</div>
							</div>
						</div><!-- end row -->
					</div>
	    ...
	    global $wp_query; ?>
			...
			<div id="page_header" class="mefe_def_header_style">
				<div class="bgback"></div>
