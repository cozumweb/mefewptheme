...
--------------------------------------------------------------------------------------------------*/
	...
	?>
        <div id="slideshow" class="notPadded <?php echo $style; ?>">
			...
			<div class="container mefe_slideshow">

                    <div data-transition="<?php echo $options['fs_transition'];?>" class="flexslider showOnMouseover mefe_normal_flex <?php echo $thumbs; ?> drop-shadow <?php echo $options['fs_shadow'];?>">
                        <ul class="slides">
						<?php
	...
	wp_enqueue_style('mefe_shadows');
	wp_enqueue_script('flex_slider');

					$mefe_normal_flex = array ( 'mefe_normal_flex' =>
						"
							(function($) {
								$(window).load(function(){
								
									function slideCompletemefe_normal_flex(args) {
										var caption = $(args.container).find('.flex-caption').attr('style', ''),
											thisCaption = $('.flexslider.mefe_normal_flex .slides > li.flex-active-slide').find('.flex-caption');
										thisCaption.animate({left:20, opacity:1}, 500, 'easeOutQuint');
									}
								
									if ( jQuery('.flexslider.mefe_normal_flex').hasClass('mefe_has_thumbs') ) {
										thumbs = 'thumbnails';
									}
									else {
										thumbs = true;
									...
									transition = jQuery(\".flexslider.mefe_normal_flex\").attr('data-transition');
									
									$(\".flexslider.mefe_normal_flex\").flexslider({
										...
										start: slideCompletemefe_normal_flex,
										after: slideCompletemefe_normal_flex
									});
								});
							})(jQuery);
						");
						
						mefe_update_array( $mefe_normal_flex );
