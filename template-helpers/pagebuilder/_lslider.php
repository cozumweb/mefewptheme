...
--------------------------------------------------------------------------------------------------*/
	...
	{	
		...
		if ( isset ( $options['ls_header_style'] ) && !empty ( $options['ls_header_style'] ) && $options['ls_header_style'] != 'mefe_def_header_style' ) { 
			$style = 'uh_'.$options['ls_header_style'];
		} else { 
			$style = 'mefe_def_header_style';
		}
			...
			<div class="laptop-slider-wrapper">
				<div class="container">
					...
					<div class="laptop-mask">
						<div class="flexslider mefe_laptop_slider">
							<ul class="slides">
	...
	$mefe_laptop_slider = array ( 'mefe_laptop_slider' =>
			"
			(function($){ 
				$(window).load(function(){
					
					function slideCompletemefe_laptop_slider(args) {
						var caption = $(args.container).find('.flex-caption').attr('style', ''),
							thisCaption = $('.flexslider.mefe_laptop_slider .slides > li.flex-active-slide').find('.flex-caption');
						thisCaption.animate({left:20, opacity:1}, 500, 'easeOutQuint');
					}
						
					$(\".flexslider.mefe_laptop_slider\").flexslider({
						...
						start: slideCompletemefe_laptop_slider,
						after: slideCompletemefe_laptop_slider
					});
				});
			})(jQuery);
			");
			
			mefe_update_array( $mefe_laptop_slider );
